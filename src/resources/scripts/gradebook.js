...
function viewGradeBook(self,classId, sectionId){
	...
	$.ajax({
		...
		success: function(data){
			$('#gradeBookContent').empty().append(data);
			updateGradeBook();
			...
			$('.eventLabel').bind('click',function(event){
				...
				$('.gradeBookStudent').each(function(){
					$(this).children().eq(index).addClass('highlighted').bind('click',function(event){event.stopPropagation();});
			...
			var oTable = $('#gradeBookTable').dataTable({
				"bPaginate": false,
				"bLengthChange": false,
				"bFilter": false,
				"bSort": false,
...
$('#statsTool').live("mouseover mouseout", function(event) {
...
$('.categorySelectAddRow').live('click', function(){
	addGradeScaleCategory($(this).parent());
	
});

$('.categorySelectDeleteRow').live('click', function(){
	deleteGradeScaleCategory($(this).parent());
	
});

$(".studentName").live("mouseover mouseout", function(event) {
	if ( event.type == "mouseover" ) {
...
$('.categoryUpper').live('keypress', function(e){
...
$('.categoryLower').live('keypress', function(e){
...
$('.eventPossiblePoints').live('keypress', function(e){
...
$('#statReturn').live('click', function(e){
		e.stopPropagation();
		closeStat();
	});

$('.eventCategory').live('keypress', function(e){
	if (e.which == 13) { 
		e.preventDefault();
	}
}).live('focusout', function(){

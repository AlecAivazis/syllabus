...
# associate number with month name
month_names = {
    1: 'January',
    2: 'February',
    3: 'March',
...
def calendarMonth(request):
    ...
    # grab a python calendar
    c = calendar.Calendar()
    # that begins on sunday
    c.setfirstweekday(calendar.SUNDAY)
    
    # save today for future reference
    today = datetime.date.today()
    
    # grab the appropriate month
    intMonth = int(number) if number else int(today.month)
    # and its name
    monthName = month_names[intMonth]
    # get the appropraite year
    year = year if year else today.year
    
    # grab the appropriate calendar
    monthList = c.monthdatescalendar(year = int(year), month=int(number))
    
    # build the calendar as a dictionary of dates as keys and event list as values
    eventDict=collections.defaultdict(list)
    # go over every class that I teach
    for qlass in Class.objects.filter(professor=request.user).all():
        # for each event
        for event in qlass.events.all():
            # if its due in this month \pm 1
            if (event.date.month == intMonth or event.date.month == intMonth-1 
                or event.date.month == intMonth+1) :
                # make sure we dont double count - some events are referenced twice
                if not event in  eventDict[str(event.date)]:
                    # add the event to the dictionary
                    eventDict[str(event.date)].append(event)
                    
    # store the registration groups a as dict where days -> (group , start | end )
    groups = defaultdict(list)
    # store the terms as a dict where days -> (term , start | end )
    termDates = defaultdict(list)
    
    # collect the groups that start this month
...
def calendarAjax(request):
    """ handle the calendar ajax request """
    which = request.GET['which']
    # if they asked for the month view
    if which =="month":
        # return it
        return calendarMonth(request)
    # they must have asked for the week view (day view tbi)
    else:
